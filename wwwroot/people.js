window.addEventListener("DOMContentLoaded", async () => {
  ...
  const registerEvents = () => {
    document
      .querySelectorAll("[data-edit-person-id]")
      .forEach((editButton) =>
        editButton.addEventListener("click", () =>
          handleEditPerson(editButton.dataset.editPersonId)
        )
      );

    document
      .querySelectorAll("[data-delete-person-id]")
      .forEach((deleteButton) =>
        deleteButton.addEventListener("click", () =>
          handleDeletePerson(deleteButton.dataset.deletePersonId)
        )
      );
  };
  ...
  const renderTable = async () => {
    ...
    people.map(
      (person) => (peopleTableBody.innerHTML += tableRow({ ...person }))
    );
    registerEvents();
  };
