window.addEventListener("DOMContentLoaded", async () => {
  ...
  const handleEditBookSubmit = async (e) => {
    ...
    try {
      ...
      if (!result.ok) {
        throw new Error("Wystąpił błąd podczas dodawania.");
      }
    } catch ({ message }) {
      alert(message);
    } finally {
  ...
  const registerEvents = () => {
    document
      .querySelectorAll("[data-edit-book-id]")
      .forEach((editButton) =>
        editButton.addEventListener("click", () =>
          handleEditBook(editButton.dataset.editBookId)
        )
      );

    document
      .querySelectorAll("[data-delete-book-id]")
      .forEach((deleteButton) =>
        deleteButton.addEventListener("click", () =>
          handleDeleteBook(deleteButton.dataset.deleteBookId)
        )
      );
  };
